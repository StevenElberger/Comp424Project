...
// Confirm that a user requested to reset password
$username = find_user_with_token($token);

if(!isset($username)) {
	
	// Token wasn't in URL or didn't match a user.
...
if(request_is_post() && request_is_same_domain()) {
  ...
  if(!csrf_token_is_valid() || !csrf_token_is_recent()) {
   ...
   log_error("Form Forgery", $log_info);
	...
	$log_info = "A User attempted to give a post request from a different domain in Reset Password. IP Address: " . $_SERVER['REMOTE_ADDR'];
